...
int checkresult(S_BOARD *pos) {
    ...
    if (pos->fiftyMove > 100) {
     printf("1/2-1/2 {fifty move rule (claimed by Caeser)}\n"); return TRUE;
    }

    if (ThreeFoldRep(pos) >= 2) {
     printf("1/2-1/2 {3-fold repetition (claimed by Caeser)}\n"); return TRUE;
    }

	if (DrawMaterial(pos) == TRUE) {
     printf("1/2-1/2 {insufficient material (claimed by Caeser)}\n"); return TRUE;
    }
	...
	if(InCheck == TRUE)	{
	    if(pos->side == WHITE) {
	      printf("0-1 {black mates (claimed by Caeser)}\n");return TRUE;
        } else {
	      printf("0-1 {white mates (claimed by Caeser)}\n");return TRUE;
        }
    } else {
      printf("\n1/2-1/2 {stalemate (claimed by Caeser)}\n");return TRUE;
    }
	return FALSE;
}
...
void Console_Loop(S_BOARD *pos, S_SEARCHINFO *info) {
	...
	printf("Welcome to Caeser v1.0 In Console Mode!\n");
